...
const { data: files, error } = await client
  .from("cre_files")
  .select("id,storage_path,size,mime,fingerprint,scan_attempts")
...
for (const f of files) {
  ...
  const saved = await client.from("cre_files").update({scan:result,scan_attempts:f.scan_attempts+1,scanned_at:new Date().toISOString()}).eq("id",f.id).eq("fingerprint",f.fingerprint);
