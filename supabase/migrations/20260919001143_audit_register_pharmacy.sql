-- Aditivo exclusivo de homologação: cadastro mínimo e auditoria na mesma transação.
begin;
create function aud_private.register_pharmacy_audit(payload jsonb) returns jsonb
...
revoke all on function aud_private.register_pharmacy_audit(jsonb) from public,anon;
grant execute on function aud_private.register_pharmacy_audit(jsonb) to authenticated;
create function public.aud_register_pharmacy(payload jsonb) returns jsonb
language sql security invoker set search_path='' as $$select aud_private.register_pharmacy_audit(payload)$$;
revoke all on function public.aud_register_pharmacy(jsonb) from public,anon;
grant execute on function public.aud_register_pharmacy(jsonb) to authenticated;
commit;
