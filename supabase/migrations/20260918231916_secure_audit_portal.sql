...
begin
 ...
 if op='detail' then
  ...
  mime=case ext when 'pdf' then 'application/pdf' when 'jpg' then 'image/jpeg' when 'jpeg' then 'image/jpeg' when 'png' then 'image/png' when 'tif' then 'image/tiff' when 'tiff' then 'image/tiff' end;
...
insert into storage.buckets(id,name,public,file_size_limit,allowed_mime_types) values('auditoria-private','auditoria-private',false,104857600,array['application/pdf','image/jpeg','image/png','image/tiff']);
